...
- [Project 2：Gitlet](#project-2gitlet)
  - [目录 {#目录}](#目录-目录)
  - [关于本说明文档的一点说明 {#关于本说明文档的一点说明}](#关于本说明文档的一点说明-关于本说明文档的一点说明)
  - [Gitlet 概览 {#gitlet-概览}](#gitlet-概览-gitlet-概览)
  - [内部结构 {#内部结构}](#内部结构-内部结构)
  - [行为的详细规范 {#行为的详细规范}](#行为的详细规范-行为的详细规范)
    - [总体规范 {#总体规范}](#总体规范-总体规范)
  - [命令 {#命令}](#命令-命令)
    - [`init`](#init)
    - [`add`](#add)
  ...
  - [骨架代码 {#骨架代码}](#骨架代码-骨架代码)
  - [设计文档 {#设计文档}](#设计文档-设计文档)
  - [评分器说明 {#评分器说明}](#评分器说明-评分器说明)
    - [阶段检查评分器 {#阶段检查评分器}](#阶段检查评分器-阶段检查评分器)
    - [完整评分器 {#完整评分器}](#完整评分器-完整评分器)
    - [Snaps 评分器 {#snaps-评分器}](#snaps-评分器-snaps-评分器)
    - [额外加分 {#额外加分}](#额外加分-额外加分)
  - [关于本项目的其他注意事项 {#关于本项目的其他注意事项}](#关于本项目的其他注意事项-关于本项目的其他注意事项)
  - [文件处理 {#文件处理}](#文件处理-文件处理)
  - [序列化细节 {#序列化细节}](#序列化细节-序列化细节)
  - [测试 {#测试}](#测试-测试)
  - [在课程组参考实现上测试 {#在课程组参考实现上测试}](#在课程组参考实现上测试-在课程组参考实现上测试)
  - [理解集成测试 {#理解集成测试}](#理解集成测试-理解集成测试)
    - [示例测试 {#示例测试}](#示例测试-示例测试)
    - [为测试准备环境 {#为测试准备环境}](#为测试准备环境-为测试准备环境)
    - [使用模式匹配输出 {#使用模式匹配输出}](#使用模式匹配输出-使用模式匹配输出)
    - [测试小结 {#测试小结}](#测试小结-测试小结)
  - [调试集成测试 {#调试集成测试}](#调试集成测试-调试集成测试)
    - [找到真正需要调试的那一次程序执行 {#找到真正需要调试的那一次程序执行}](#找到真正需要调试的那一次程序执行-找到真正需要调试的那一次程序执行)
  - [远程功能（额外加分） {#远程功能额外加分}](#远程功能额外加分-远程功能额外加分)
  - [远程命令 {#远程命令}](#远程命令-远程命令)
    ...
    - [`fetch`](#fetch)
    - [`pull`](#pull)
  - [I. 应避免的做法 {#i-应避免的做法}](#i-应避免的做法-i-应避免的做法)
  - [J. 致谢 {#j-致谢}](#j-致谢-j-致谢)
